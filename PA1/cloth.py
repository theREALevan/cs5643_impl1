...
# Collision Obstacle
obstacle = Scene(Init.CLOTH_TABLE)  # Use CLOTH_TABLE configuration
contact_eps = 1e-4
record = False
...
def timestep():
    ...
    for i, j in ti.ndrange(n, n):
        ...
        # collision with circular disk table
        
        diff = x[i,j] - obstacle.tabletop_center[0]
        dist = diff.norm() + 1e-6
        
        if x[i, j][1] <= obstacle.tabletop_center[0]:
                if dist < obstacle.tabletop_radius + 1e-4:
                    n_dir = diff / dist
                    v[i, j] = v[i, j] - ti.min(0.0, v[i, j].dot(n_dir)) * n_dir
                    x[i, j] = obstacle.tabletop_center[0] - n_dir * (obstacle.tabletop_radius + 1e-4)
